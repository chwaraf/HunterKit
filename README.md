...
Current version: **0.9.77** — see [`CHANGELOG.md`](CHANGELOG.md).
...
| **What to press next** | Opt-in row under the bar (**Show a 'what to press next' row**, off by default). Rather than only showing clocks it names the button worth pressing: the two-mob macro first, then **AIMED SHOT**, **MULTI-SHOT**, **RAPTOR STRIKE** when you are standing in melee, then **weave** or **hold**. The approach Fluffy Hunter Bars takes — useful while the rotation is still being learned, easy to switch off once it is automatic. The lockout also grows a **latency end-slice**: the red zone is drawn from the fixed 0.5s cast, but the boundary that actually costs you damage is 0.5s *plus your own latency*, which is precisely what the `+0.34s` readout measures — so the measured tail is painted into the bar, turning a number you have to read into a region you can see. The **auto shot and melee swing bars are the same height** — in the two-mob setup the two cycles matter equally, so they read as two equal bars rather than a bar and a caption. Bar height now defaults to **27px** (was 18) and the slider reaches 60; profiles still on the untouched 18 are migrated, a height you chose yourself is never rewritten. The state line underneath is a fixed caption and does **not** grow with that slider. |
...
Add `--verbose` to also echo the addon's chat output. **1057 checks**, in **9** files:
