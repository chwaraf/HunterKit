...
|---|---|
| **Feed Pet button** | A one-click button beside the pet happiness icon. Always feeds the **best** food in your bags (max happiness tier, then smallest open stack), shown as the food icon (or the **default Feed Pet icon** via the *Use default Feed Pet icon* option) with a **count of the food you can actually feed** on it like an action button — every stack of every food this pet eats that scores the **same happiness tier**, so eight different appropriate-level meats with one item each read **8**, not 1. Food the pet cannot eat is never in the number, and neither is grey-level food a tier down, because a bite of it is worth less. Hovering says how many different foods made the number up, and whether a feed is worth it — a **Happy** pet is told it will still eat but that there is little happiness left to gain, and **combat** is named as the thing that actually blocks a feed. While the pet is **eating**, the button swaps its count for the **happiness each bite is worth** (`+35`, `+17` or `+8` by how close the food is to the pet's level) and shows the **seconds left** just to the right — read from the *Feed Pet Effect* buff, so a feed cancelled by damage stops counting down the moment the game does. The highlight glows only when the pet is **below happy and you are out of combat**. **Drag any food onto the button** to pin it and teach the button that your pet eats it — there is no game API for what diet a food belongs to, so the built-in list can never cover every cooked dish, and this closes the gap. Hold **Shift** over the button and a panel of **icons, one below the other**, lists every food in your bags — what it will feed first, then the rest — so you can see at a glance what you are carrying and drop one to pin it. Shift works whether you press it before or after hovering. Taught foods carry the diet that was in force when you dropped them, so switching pets does not resurrect food the new pet cannot eat. Respects your pins/excludes — and a pin made for a *different* pet is refused once the current pet's diet contradicts it, instead of being named in the tooltip and fed on click. Out-of-combat only (as Blizzard intends). |
...
| **Melee weave marker** | Part of the shot bar. In **Classic Era** the melee and ranged swing timers are **independent** — that is what makes melee weaving possible, and it is worth real damage. (This is version-specific: WotLK deliberately *linked* them, which killed weaving there. HunterKit targets Era.) Out of the box the bar advises **static weaving only** and never tells you to go running anywhere — but note it will still show the blue marker whenever a mob is *already* within swing distance, and "within swing distance" means any attackable mob inside the ~11yd probe, not only your target: your **mouseover**, your **pet's target** and whatever is **attacking your target** all count, because in the two-mob setup any of those can be the mob at your feet. So the marker can come and go as mobs drift in and out while the option is off. That is the static weave, not the travel option leaking — it is pinned by tests as a deliberate decision. Turning on **Also weave by running in and out** (opt-in) adds the travel case — a blue line marking the last moment you could still leave, land a swing and be back before the shot, with the label reading **WEAVE** while the round trip genuinely fits. A second thin strip tracks your **melee swing**, driven by swings actually observed in the combat log rather than assumed — so it never claims a swing you did not get. It will not suggest a weave when the melee swing would still be on cooldown when you arrive, nor when the free window is too short for the trip (which is what a haste proc does to it), nor — by default — while Aimed Shot or Multi-Shot is up, since either is worth more than a Raptor Strike (that gate is switchable for max-weavers). The Aimed/Multi-Shot cooldown pips themselves are a separate opt-in row (**Show Aimed / Multi-Shot cooldowns**, off by default). While weaving is on the bar stays up for the whole fight, including in melee range where auto-shot stops -- that is exactly when the melee row matters. The melee strip is drawn from the moment weaving is on, not only after your first swing, because the common speedrun pattern is to shoot a pet-held target at range while meleeing a second one next to you. The options window draws a labelled picture of the bar so you can learn the colours without hovering anything, and the bar can optionally be **kept on screen** instead of appearing and vanishing. The round-trip time is a setting (default **2.5s**, the figure the Classic hunter guides quote for a good hunter with a movement buff) because the marker is only honest if it matches how fast *you* actually move — and it is ignored entirely while the target is already in melee. |
| **Two-mob weave indicator** | Part of the shot bar. For the setup where your **pet holds one mob at range** and you stand in **melee of a second one**, swinging at it between shots — the exact situation the **Two-mob weave** macro exists for, and one the bar previously said nothing about (its only marker was the travel-weave departure point above). A state strip under the bar reads **PRESS 2-MOB** (green) the moment pressing that macro is correct, **2-MOB - SWING 1.2s** (blue) while the setup is live but the swing is still coming, **MELEE ONLY** (amber) when something is in melee but there is no second mob to shoot, **RANGE** (grey) when nothing is in melee, and **OUT OF RANGE** (red) when your target is past Auto Shot range. "Correct" is not guessed — it is every leg of what the macro actually does, read live: your target is alive and *within melee* (or `/startattack` hits nothing), your pet has a *different* live target (or every `[@pettarget]` line is skipped and the flick is a no-op), your melee swing is up (or the press only restarts `/startattack`), and Auto Shot is out of its 0.5s lockout (or the `/cast !Auto Shot` inside the flick is the clip). **Aimed and Multi-Shot deliberately do not gate it** — they are spent on the mob you are *shooting*, which is a different decision from whether to swing at the one next to you; they still gate the separate travel-weave advice, where the trade-off is real. Opt-in **Show the two-mob press icon** adds a separate, independently draggable icon — bright with **NOW** on it at the press moment, dimmed with a countdown while the swing is coming, nearly invisible otherwise, so a flash means press. It parks beside the bar until you move it with `/htk unlock`. |
| **What to press next** | Opt-in row under the bar (**Show a 'what to press next' row**, off by default). Rather than only showing clocks it names the button worth pressing: the two-mob macro first, then **AIMED SHOT**, **MULTI-SHOT**, then **weave** or **hold**. The approach Fluffy Hunter Bars takes — useful while the rotation is still being learned, easy to switch off once it is automatic. The lockout also grows a **yellow clip slice** immediately to its left: the red zone is drawn from the fixed 0.5s cast, but the honest boundary is later than that, and by how much is precisely what the `+0.34s` readout measures — so the measured tail is painted into the bar, turning a number you have to read into a region you can see. It is **not** a latency reading, and nothing in the addon reads your connection: the width is how late your *last* shot actually landed, which latency, spell batching, the server's re-shot timer and — mostly — acting inside the lockout can all cause. Read it as *how much you clipped*. It lags one cycle, because a shot cannot be known to have been late until it arrives, and clears on the next clean shot. The **auto shot and melee swing bars are the same height** — in the two-mob setup the two cycles matter equally, so they read as two equal bars rather than a bar and a caption. Bar height now defaults to **27px** (was 18) and the slider reaches 60; profiles still on the untouched 18 are migrated, a height you chose yourself is never rewritten. The state line underneath is a fixed caption and does **not** grow with that slider. |
| **Options** | Draggable settings window — a master **Enable HunterKit** switch that pauses *every* feature at once, one rule per feature block, wrapped tooltips, and a two-click **Reset ALL settings** button (it arms to *Click again to CONFIRM* and disarms itself after 5 s). Thirteen sections with a **button for each down the left edge**, so jumping to *Ammo auto-buy* is one click instead of a scroll-and-scan: Master, Feed Pet, Sniper Mark, Pet Mend Marker, Ammo, Ammo auto-buy, Pet aggro warning, Weapon timers, Gun Sound, Passive pet alert, Macros, Positions (**lock/unlock** + **Reset positions** buttons) and Reset. The Sniper Mark block is a grid per state: **SHAPE** cycle-button + state name on the left, short **BRIGHTNESS** slider (0–200%, 100% at the bar's middle, overdrive stacks a second additive pass) on the right. Minimap button, `/htk lock\|unlock`, `/htk reset`. |
...
The mark is drawn **additively**: its transparent surround adds nothing and its
strokes add their colour at full strength, which is why it reads so well over
dark art. The catch is the one you hit on snow, open sky or white sand - adding
light to light is still light, so no brightness setting can separate the mark
from a bright background. Raising the glow makes it worse.

Only something that *replaces* the background can fix that. Options -> Sniper
Mark -> **Contrast outline** draws the same art again underneath in ordinary
BLEND, offset in eight directions, so the offsets dilate the strokes into a
silhouette:

- **Off** (default) leaves the mark exactly as it was.
- **Dark** puts a black rim under it - the one you want on a bright background.
  On dark art the rim is invisible, so nothing changes there.
- **Light** puts a white rim under it, which boldens the mark against dark art.
- **Outline width** (1-6 px) is how far the rim extends past the mark. 1-2 reads
  as a crisp edge, 5-6 as a halo.

One setting covers all three states, and the additive mark on top is untouched:
its colour, its brightness slider and its overdrive pass all behave exactly as
before. Overdrive never stacks a second rim.
...
- **Master preset** cycles `Current` -> `Below the UI` -> `Normal` -> `Above the UI` ->
  `Always on top`. `Current` is the default and means *exactly the layer each
  frame was built with*, so cycling through the presets and back leaves every
...
`Always on top` deliberately stops at `DIALOG` rather than `FULLSCREEN`: the
full-screen panels - the world map above all - still cover HunterKit, so no
setting can reintroduce the marker-over-the-map problem fixed in 0.9.89.

Only the HUD frames are covered. The settings window, the macro library and the
shift-hover food panel keep their dialog layers, because they are meant to sit on
top of whatever you are doing.
